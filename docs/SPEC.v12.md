...
**Mới trong bản này:** (1) UX trang Kiểm duyệt viết lại — hàng rộng full-width thay vì thẻ nhỏ, thêm 2 action rõ ràng "Duyệt" và "Từ chối" thay cho auto-mark-khi-mở-trang (mục 2.9, 2.9a); (2) ghi nhận bug board không đổi màu sắc dù đã đổi skin (mục 2.6 cập nhật, cần Cursor debug); (3) yêu cầu Cursor xác minh lại Phase 5.15 (trang Tài khoản cá nhân) vì vẫn chưa thấy trên UI.
...
### 2.2a Tài khoản cá nhân (mọi user, không cần quyền admin)

Mỗi user tự quản lý thông tin của chính mình — không cần quyền Admin để dùng trang này.

- [ ] **Đổi tên hiển thị** — user tự sửa `display_name` của chính mình
- [ ] **Đổi mật khẩu** — nhập mật khẩu hiện tại để xác nhận + mật khẩu mới (2 lần để tránh gõ sai) — yêu cầu xác nhận mật khẩu cũ để an toàn hơn (tránh trường hợp máy đang đăng nhập sẵn bị người khác đổi mật khẩu hộ)
- [ ] **Xem danh sách phòng ban mình đang thuộc** — chỉ xem, không tự thêm/bớt (việc gán phòng ban vẫn do Admin quản lý ở mục 2.2)
- [ ] Truy cập qua menu tên/avatar user ở góc nav (thay vì chỉ có nút "Đăng xuất" trơ trọi) — bấm vào mở dropdown gồm "Tài khoản của tôi" + "Đăng xuất"
...
- [ ] Khi tạo hoặc cài đặt lại 1 board: có thể **gán 1 hoặc nhiều thành viên làm Board Admin** cho đúng board đó (xem mục 2.3a)

### 2.3a Board Admin — quyền admin giới hạn theo 1 board

Vai trò mới, **khác với Admin công ty** (`members.role = 'admin'`) — đây là quyền được **gán riêng cho từng board**, một người có thể là Board Admin của board này nhưng là user thường ở board khác.

**Board Admin được phép** (chỉ trong đúng board được gán):
- [ ] Ẩn/xoá ghim vi phạm trong board đó (đúng cơ chế `is_hidden` ở mục 2.9, không phải tính năng mới)
- [ ] Xem danh tính thật đằng sau ghim ẩn danh trong board đó (để xử lý vi phạm)
- [ ] Đổi loại bảng (skin) của board đó
- [ ] Bật/tắt nhúng (embed) + lấy mã iframe của board đó (mục 2.8)
- [ ] Thấy trang Kiểm duyệt (mục 2.9), nhưng **chỉ lọc trong phạm vi board mình được gán** — không thấy ghim của board khác nếu không được gán

**Board Admin KHÔNG được phép:**
- [ ] Quản lý danh sách thành viên toàn công ty (thêm/xoá user)
- [ ] Thêm/bớt thành viên khỏi phòng ban (kể cả phòng ban gắn với board mình quản lý)
- [ ] Tạo/giải thể phòng ban khác
- [ ] Đổi tên/logo công ty
- [ ] Ẩn ghim hoặc đổi cài đặt ở board **khác** không được gán
- [ ] Vào Platform Admin (Connecta)
- [ ] Không thấy nav "Cài đặt" (Thành viên/Phòng ban/Cài đặt công ty) nếu không đồng thời là Admin công ty — chỉ thấy "Kiểm duyệt" (đã lọc) + icon cài đặt riêng của board mình quản lý
...
- [ ] Viết nội dung — **tuỳ chọn nếu đã có ảnh, bắt buộc nếu không có ảnh**: phải có ít nhất 1 trong 2 (nội dung hoặc ảnh), không cho đăng ghim hoàn toàn trống
- [ ] Chọn **template thẻ**: 7 mẫu (xem mục 3.5)
...
### 2.5a Sửa Ghim (chỉ người đăng gốc)
- [ ] Chỉ **chính người đăng** mới sửa được ghim của mình — kể cả Admin cũng không sửa nội dung của người khác (Admin chỉ có quyền ẩn, không có quyền viết lại lời người khác)
- [ ] Chỉ sửa được: **nội dung chữ, ảnh, template** — không sửa được sau khi đăng: công khai/ẩn danh, người nhận, bảng đích (giữ nguyên "sự thật cấu trúc" của ghim, tránh đổi ẩn danh↔công khai để né trách nhiệm)
- [ ] Không giới hạn thời gian sửa
- [ ] Ghim đã sửa hiện nhãn nhỏ **"(đã chỉnh sửa)"** kèm thời điểm sửa gần nhất — giống Messenger/Facebook, để người xem biết đây không còn là bản gốc 100%
- [ ] Ghim đang bị Admin ẩn (`is_hidden = true`) thì **không sửa được** — phải chờ Admin xử lý trước
- [ ] Vẫn áp validate template khi sửa: đổi sang Polaroid mà không có ảnh → chặn, đúng như lúc tạo mới
- [ ] Vẫn áp quy tắc "ít nhất 1 trong 2 (nội dung/ảnh)" khi sửa — không cho sửa thành trống cả 2

### 2.5b Cải thiện UI/UX form tạo & sửa ghim
- [ ] Ô chọn "Mẫu ghim" phải là **mini-render thật** của từng `<PinCard>` (thu nhỏ, đúng texture/trang trí theo `docs/pin-templates-reference.html`) — không phải ô màu phẳng
- [ ] Ô Polaroid: nếu chưa có ảnh, hiện **badge khoá đè trực tiếp lên ô** (VD: 🔒 "Cần ảnh") thay vì dòng chữ trôi nổi riêng phía dưới danh sách
- [ ] Thứ tự field trong form: **Ảnh → Nội dung → Mẫu ghim → Bảng đích → Người nhận (tuỳ chọn) → Đăng ẩn danh** — Ảnh lên đầu tiên, vì cả việc nội dung có bắt buộc hay không lẫn việc Polaroid mở/khoá đều phụ thuộc vào đã có ảnh hay chưa
- [ ] Đổi nhãn field ảnh từ "Ảnh đính kèm" thành **"Ảnh"** — ngắn gọn hơn
- [ ] Placeholder ô nội dung cập nhật gợi ý: có thể để trống nếu đã đính kèm ảnh (VD: "Viết lời biết ơn, kỷ niệm... (có thể bỏ trống nếu đã có ảnh)")
- [ ] Nút chọn ảnh + dropdown "Bảng đích"/"Người nhận" + checkbox "Đăng ẩn danh" style lại theo design system (bo góc, tông pastel, font đã chọn ở mục 3.3) — không dùng control mặc định thô của trình duyệt
- [ ] Sau khi chọn ảnh, hiện thumbnail ảnh đã chọn ngay trong form (không chỉ tên file dạng "No file chosen")
- [ ] **Không cần khung xem trước trực tiếp (live preview) riêng** — mini-render trên ô chọn template đã đủ để hình dung, giữ form đơn giản, gọn
- [ ] Nút "Đăng ghim" disable nếu cả nội dung lẫn ảnh đều trống, kèm gợi ý ngắn giải thích vì sao
...
- [ ] **Icon "⚙️ Cài đặt bảng"** hiển thị rõ ràng trên header của board (cạnh tên bảng, VD ngay cạnh badge "Bảng chung · Gỗ mộc") — chỉ hiện với company Admin hoặc Board Admin của đúng board đang xem, dẫn tới trang cài đặt board (đổi skin, bật/tắt embed, gán Board Admin — đã có ở mục 2.3a). **Áp dụng cho MỌI board, kể cả Bảng chung công ty** — không chỉ board phòng ban
- [ ] ⚠️ **Bug ghi nhận**: đổi skin ở trang cài đặt board có lưu vào DB (badge header đổi tên đúng, VD "Bảng chung · Chalkboard") nhưng **nền/kết cấu board hiển thị không đổi theo** — vẫn giữ giao diện cũ. Cursor cần debug: component render nền board có đang đọc đúng giá trị `boards.skin` mới nhất không, hay đang dùng giá trị cache/props cũ chưa refetch sau khi đổi skin.
...
### 2.6d Ẩn ghim ngay trên bảng (Admin) — không cần qua trang Kiểm duyệt riêng
- [ ] Nếu người đang xem board là **Admin của công ty đó**, mỗi ghim (chưa bị ẩn) có thêm lựa chọn **"Ẩn ghim"** trong menu tuỳ chọn sẵn có trên thẻ (menu "...")
- [ ] User thường (không phải Admin) **không thấy** lựa chọn này — kể cả trên ghim của chính mình (người đăng không tự xoá được, đúng nguyên tắc "kỷ niệm không biến mất")
- [ ] Bấm "Ẩn ghim" → hỏi xác nhận ngắn gọn (1 bước, không cần nhập lý do) → ghim biến mất khỏi board ngay lập tức cho mọi người
- [ ] Đây là **lối tắt** cho hành động đã có ở trang Kiểm duyệt (mục 2.9) — cùng 1 cơ chế `is_hidden`, không phải tính năng mới về mặt dữ liệu, chỉ thêm điểm truy cập nhanh ngay tại board

### 2.6b Menu "Bảng ghim" — dropdown chọn bảng + đặt mặc định
- [ ] Nav item "Bảng ghim" mặc định dẫn tới **bảng mặc định của người dùng đó** (không phải luôn luôn là Bảng chung công ty)
- [ ] Hover/click vào "Bảng ghim" → xổ xuống danh sách toàn bộ bảng mà user đang xem được (Bảng chung + các bảng phòng ban mình thuộc về)
- [ ] Mỗi dòng trong dropdown có thể **tick chọn "Đặt làm mặc định"** — lần sau bấm "Bảng ghim" sẽ vào thẳng bảng đó
- [ ] Nếu chưa từng đặt mặc định: mặc định là Bảng chung công ty

### 2.6c Menu "Cài đặt" — gộp Thành viên / Phòng ban / Cài đặt
- [ ] 3 mục nav riêng lẻ "Thành viên", "Phòng ban", "Cài đặt" gộp chung thành 1 nav item **"Cài đặt"**
- [ ] Chỉ hiện với người có quyền xem (Admin) — user thường không thấy mục "Cài đặt" này (giữ đúng phân quyền hiện có, chỉ đổi cách hiển thị nav)
- [ ] Hover/click vào "Cài đặt" → xổ xuống 3 mục con: Thành viên, Phòng ban, Cài đặt (route và chức năng bên trong giữ nguyên như cũ, chỉ gộp lối vào)
...
### 2.9 Kiểm duyệt (Admin / Board Admin đúng phạm vi)
- [ ] Xem toàn bộ ghim (kể cả ẩn danh — admin/board admin thấy được ai đăng để xử lý vi phạm, nhưng không hiển thị ra ngoài)
- [ ] **Giao diện dạng danh sách hàng ngang, full-width** — mỗi ghim là 1 hàng rộng (không phải thẻ nhỏ xếp dọc như hiện tại), đủ chỗ hiển thị: tên bảng, người đăng, nội dung, thời gian đăng, và 2 nút hành động
- [ ] **2 hành động rõ ràng trên mỗi hàng:**
  - **"Duyệt"** — xác nhận ghim ổn, không vi phạm gì. Chỉ đánh dấu đã xem (`reviewed_at`), **ghim vẫn giữ nguyên trên board, không bị ẩn**. Ghim biến mất khỏi danh sách Kiểm duyệt sau khi Duyệt.
  - **"Từ chối"** — ghim vi phạm. Đổi trạng thái ẩn (`is_hidden = true`), ghim **biến mất khỏi board công khai ngay lập tức**, đồng thời cũng biến mất khỏi danh sách Kiểm duyệt (đã xử lý xong, không cần rà soát lại)
- [ ] (Không có bước duyệt **trước khi** đăng — ghim vẫn lên board ngay lập tức khi đăng, "Duyệt"/"Từ chối" ở đây là xử lý **sau khi** đã đăng, giữ trải nghiệm đăng ghim nhanh gọn)

> **Nguyên tắc cốt lõi**: Ghim đã đăng không thể tự xoá bởi chính người đăng — đúng tinh thần "kỷ niệm không biến mất". Chỉ Admin/Board Admin mới có quyền ẩn ("Từ chối"), và chỉ dùng cho trường hợp vi phạm.

### 2.9a Badge cảnh báo trên menu "Kiểm duyệt"
- [ ] Nav item "Kiểm duyệt" hiện **icon chấm than đỏ** khi còn ghim nào chưa được xử lý (chưa bấm "Duyệt" hoặc "Từ chối")
- [ ] Nhắc lại nguyên tắc: ghim đăng lên **luôn lên bảng ngay lập tức** — trang Kiểm duyệt chỉ là **danh sách rà soát**, không phải cổng chặn
- [ ] **Khác v6**: badge **không** tự động biến mất chỉ vì mở trang xem qua — phải xử lý **từng ghim cụ thể** (bấm Duyệt hoặc Từ chối) thì ghim đó mới rời khỏi danh sách và giảm số đếm badge. Mở trang mà không bấm gì thì badge vẫn còn nguyên.
