...
**Mới trong bản này:** Phase 5.8 — sửa ghim sau khi đăng (chỉ tác giả gốc, nhãn "đã chỉnh sửa"). Phase 5.9 — cải thiện UI/UX form tạo/sửa ghim (mini-render template thật, khoá Polaroid, thứ tự field, style control). Các phase khác giữ nguyên từ v4.

Làm tuần tự từ Phase 0 → Phase 9, không nhảy cóc. Sau mỗi task, chạy thử và xác nhận hoạt động trước khi sang task kế. Đọc `docs/AGENT_RULES.v5.md` và `docs/SPEC.v5.md` trước khi bắt đầu.
...
- [ ] 0.5 Setup Tailwind theme: khai báo màu trong `tailwind.config.ts` theo đúng mục 8 của `docs/AGENT_RULES.v5.md` (đặt tên token: `cream`, `peach`, `butter`, `mint`, `umber`)
- [ ] 0.6 Commit theo đúng format ở `docs/AGENT_RULES.v5.md` mục 10 — báo cáo: project chạy được `npm run dev`, Tailwind + font hiển thị đúng

---

## Phase 1 — Database schema & RLS

- [ ] 1.1 Viết migration SQL tạo đủ 6 bảng: `companies`, `members`, `departments`, `member_departments`, `boards`, `pins` — đúng schema ở `docs/AGENT_RULES.v5.md` mục 4
...
- [ ] 2.1 Trang `/signup`: form tạo công ty mới — tên công ty, upload logo (resize theo mục 6 của `docs/AGENT_RULES.v5.md`), email + mật khẩu admin đầu tiên
...
- [ ] 2.6.1 Migration: thêm cột `onboarding_completed` vào `companies`, `is_owner` vào `members` (xem `docs/AGENT_RULES.v5.md` mục 4)
...
- [ ] 2.6.3 Trang `/[companySlug]/setup`: wizard nhiều bước (component step-by-step, không cần route riêng cho từng bước) theo đúng `docs/SPEC.v5.md` mục 2.1a:
...
- [ ] 4.1 Component `<Board>`: nhận `skin` prop, render nền tương ứng (wood/felt/linen/chalkboard) — dùng CSS/texture nhẹ theo mô tả ở `docs/SPEC.v5.md` mục 3.4
...
- [ ] 5.1 Component `<PinCard>`: 4 biến thể template (note/polaroid/floral/washi) theo mô tả `docs/SPEC.v5.md` mục 3.5, mỗi biến thể có prop nhận ảnh (optional) + nội dung
...
Đây là phase sửa lại phần đã build ở Phase 4-5, không phải làm mới từ đầu. Đọc kỹ `docs/SPEC.v5.md` mục 2.6, 2.6a, 3.5 (đã cập nhật) trước khi làm.

- [ ] 5.5.1 Migration: thêm cột `position_x`, `position_y`, `rotation` vào bảng `pins` (xem `docs/AGENT_RULES.v5.md` mục 4)
- [ ] 5.5.2 Viết Postgres function `update_pin_position` (SECURITY DEFINER) theo đúng mục 5.1 của `docs/AGENT_RULES.v5.md`, không mở UPDATE chung cho bảng `pins`
...
- [ ] 5.5.7 Lúc tạo ghim mới: tự sinh `position_x/position_y` hợp lý (tránh chồng lên ghim khác trong viewport hiện tại) + `rotation` ngẫu nhiên theo đúng biên độ từng template ở `docs/SPEC.v5.md` mục 3.5
- [ ] 5.5.8 Thiết kế lại thật sự 4 `<PinCard>` variant theo đúng chi tiết ở `docs/SPEC.v5.md` mục 3.5 (kết cấu nền, khung/viền, trang trí SVG, font riêng, xử lý ảnh riêng từng loại) — không chỉ đổi màu nền
...
- [ ] 5.7.1 Migration: cập nhật CHECK constraint cột `pins.template` thêm `'garden'`, `'sunshine'`, `'love'` (xem `docs/AGENT_RULES.v5.md` mục 4)
...
- [ ] 5.7.6 Áp đúng biên độ xoay ngẫu nhiên riêng từng template khi tạo ghim mới theo `docs/SPEC.v5.md` mục 3.5 (garden -2°→2°, sunshine -4°→4°, love -3°→3°)
...
- [ ] 5.8.1 Migration: thêm cột `is_edited`, `edited_at` vào bảng `pins` (xem `docs/AGENT_RULES.v5.md` mục 4)
- [ ] 5.8.2 Viết Postgres function `update_pin_content` (SECURITY DEFINER) theo đúng mục 5.2 của `docs/AGENT_RULES.v5.md` — chỉ tác giả gốc, chỉ 3 trường content/image_url/template, validate lại Polaroid cần ảnh
...
- [ ] 5.9.3 Sắp lại thứ tự field: Nội dung → Ảnh đính kèm → Mẫu ghim → Bảng đích → Người nhận → Đăng ẩn danh (đúng `docs/SPEC.v5.md` mục 2.5b)
- [ ] 5.9.4 Style lại input file, dropdown "Bảng đích"/"Người nhận", checkbox "Đăng ẩn danh" theo design token ở `docs/AGENT_RULES.v5.md` mục 8 — không dùng control mặc định trình duyệt
...
- [ ] 9.1 Rà lại toàn bộ UI copy (text hiển thị) đúng tiếng Việt, đúng giọng ấm áp như mô tả ở `docs/SPEC.v5.md` mục 3.1
...
- [ ] 9.6 Commit cuối — báo cáo tổng kết: những gì đã hoàn thành, những gì còn để Phase 2 (Odoo sync, upload nền tuỳ ý, export zip trả phí — xem `docs/SPEC.v5.md` mục 4)
