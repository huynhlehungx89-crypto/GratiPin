...
**Mới trong bản này:** cập nhật rule validate — nội dung chữ (`content`) không còn bắt buộc nếu đã có `image_url`, chỉ bắt buộc có ít nhất 1 trong 2 (mục 5.2 và ghi chú tạo ghim mới).

Đây là quy tắc bắt buộc khi build sản phẩm này. Đọc kỹ trước khi làm bất kỳ task nào trong `docs/TASKS.v7.md`. Nếu có mâu thuẫn giữa task cụ thể và rule ở đây, **rule ở đây luôn thắng**.
...
SaaS đa khách hàng (multi-tenant): nền tảng ghi nhận, biết ơn và lưu giữ kỷ niệm công ty, giao diện dạng bảng ghim (corkboard). Xem chi tiết tính năng & design tại `docs/SPEC.v7.md` (đính kèm cùng bộ này) — file đó là nguồn sự thật (source of truth) về nghiệp vụ.
...
Bất kỳ member nào trong công ty cũng được phép kéo-thả để đổi vị trí ghim (theo `docs/SPEC.v7.md` mục 2.6a), **nhưng không được để họ sửa được nội dung/ảnh/danh tính ghim của người khác qua cùng đường đó**. Vì vậy:
...
Theo `docs/SPEC.v7.md` mục 2.5a: chỉ tác giả gốc (kể cả Admin cũng không được sửa ghim người khác) mới sửa được **content, image_url, template** — không sửa được `is_anonymous`, `recipient_member_id`, `board_id`.
...
- Đây chỉ là đánh dấu "đã lướt qua", không phải trạng thái duyệt/từ chối — ghim vẫn luôn public ngay khi đăng (đúng `docs/SPEC.v7.md` mục 2.9a), `reviewed_at` không ảnh hưởng gì tới việc ghim có hiển thị trên board hay không
...
Tham chiếu đầy đủ tại `docs/SPEC.v7.md` mục 3. Tóm tắt bắt buộc:
...
- 4 board skin: wood / felt / linen / chalkboard — mỗi skin có bộ màu đinh ghim riêng (xem docs/SPEC.v7.md mục 3.4)
- 7 pin template: note / polaroid / floral / washi / garden / sunshine / love (xem docs/SPEC.v7.md mục 3.5)
...
**Không tự ý commit** nếu chưa hoàn thành 1 task trọn vẹn trong `docs/TASKS.v7.md` và chưa báo cáo lại kết quả (đã test được gì, còn thiếu gì) trước khi commit.

---

## 11. Thứ tự làm việc

Luôn làm theo đúng thứ tự trong `docs/TASKS.v7.md`, không nhảy cóc. Sau mỗi task, chạy thử (`npm run dev` hoặc test tương ứng) trước khi chuyển task tiếp theo. Nếu 1 task bị chặn vì thiếu quyết định nghiệp vụ chưa có trong `docs/SPEC.v7.md`, dừng lại và hỏi thay vì tự đoán.
