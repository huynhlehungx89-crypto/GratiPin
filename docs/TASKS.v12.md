...
**Mới trong bản này:** Phase 5.16 — debug bug board không đổi màu dù đã đổi skin. Phase 5.17 — xác minh lại Phase 5.15 (trang Tài khoản cá nhân vẫn chưa thấy trên UI). Phase 6b — viết lại UX trang Kiểm duyệt: hàng rộng full-width + 2 nút "Duyệt"/"Từ chối" thay cho auto-mark khi mở trang.

Làm tuần tự từ Phase 0 → Phase 9, không nhảy cóc. Sau mỗi task, chạy thử và xác nhận hoạt động trước khi sang task kế. Đọc `docs/AGENT_RULES.v12.md` và `docs/SPEC.v12.md` trước khi bắt đầu.
...
- [ ] 0.5 Setup Tailwind theme: khai báo màu trong `tailwind.config.ts` theo đúng mục 8 của `docs/AGENT_RULES.v12.md` (đặt tên token: `cream`, `peach`, `butter`, `mint`, `umber`)
- [ ] 0.6 Commit theo đúng format ở `docs/AGENT_RULES.v12.md` mục 10 — báo cáo: project chạy được `npm run dev`, Tailwind + font hiển thị đúng

---

## Phase 1 — Database schema & RLS

- [ ] 1.1 Viết migration SQL tạo đủ 6 bảng: `companies`, `members`, `departments`, `member_departments`, `boards`, `pins` — đúng schema ở `docs/AGENT_RULES.v12.md` mục 4
...
- [ ] 2.1 Trang `/signup`: form tạo công ty mới — tên công ty, upload logo (resize theo mục 6 của `docs/AGENT_RULES.v12.md`), email + mật khẩu admin đầu tiên
...
- [ ] 2.6.1 Migration: thêm cột `onboarding_completed` vào `companies`, `is_owner` vào `members` (xem `docs/AGENT_RULES.v12.md` mục 4)
...
- [ ] 2.6.3 Trang `/[companySlug]/setup`: wizard nhiều bước (component step-by-step, không cần route riêng cho từng bước) theo đúng `docs/SPEC.v12.md` mục 2.1a:
...
- [ ] 4.1 Component `<Board>`: nhận `skin` prop, render nền tương ứng (wood/felt/linen/chalkboard) — dùng CSS/texture nhẹ theo mô tả ở `docs/SPEC.v12.md` mục 3.4
...
- [ ] 5.1 Component `<PinCard>`: 4 biến thể template (note/polaroid/floral/washi) theo mô tả `docs/SPEC.v12.md` mục 3.5, mỗi biến thể có prop nhận ảnh (optional) + nội dung
...
Đây là phase sửa lại phần đã build ở Phase 4-5, không phải làm mới từ đầu. Đọc kỹ `docs/SPEC.v12.md` mục 2.6, 2.6a, 3.5 (đã cập nhật) trước khi làm.

- [ ] 5.5.1 Migration: thêm cột `position_x`, `position_y`, `rotation` vào bảng `pins` (xem `docs/AGENT_RULES.v12.md` mục 4)
- [ ] 5.5.2 Viết Postgres function `update_pin_position` (SECURITY DEFINER) theo đúng mục 5.1 của `docs/AGENT_RULES.v12.md`, không mở UPDATE chung cho bảng `pins`
...
- [ ] 5.5.7 Lúc tạo ghim mới: tự sinh `position_x/position_y` hợp lý (tránh chồng lên ghim khác trong viewport hiện tại) + `rotation` ngẫu nhiên theo đúng biên độ từng template ở `docs/SPEC.v12.md` mục 3.5
- [ ] 5.5.8 Thiết kế lại thật sự 4 `<PinCard>` variant theo đúng chi tiết ở `docs/SPEC.v12.md` mục 3.5 (kết cấu nền, khung/viền, trang trí SVG, font riêng, xử lý ảnh riêng từng loại) — không chỉ đổi màu nền
...
- [ ] 5.7.1 Migration: cập nhật CHECK constraint cột `pins.template` thêm `'garden'`, `'sunshine'`, `'love'` (xem `docs/AGENT_RULES.v12.md` mục 4)
...
- [ ] 5.7.6 Áp đúng biên độ xoay ngẫu nhiên riêng từng template khi tạo ghim mới theo `docs/SPEC.v12.md` mục 3.5 (garden -2°→2°, sunshine -4°→4°, love -3°→3°)
...
- [ ] 5.8.1 Migration: thêm cột `is_edited`, `edited_at` vào bảng `pins` (xem `docs/AGENT_RULES.v12.md` mục 4)
- [ ] 5.8.2 Viết Postgres function `update_pin_content` (SECURITY DEFINER) theo đúng mục 5.2 của `docs/AGENT_RULES.v12.md` — chỉ tác giả gốc, chỉ 3 trường content/image_url/template, validate lại Polaroid cần ảnh
...
- [ ] 5.9.3 Sắp lại thứ tự field: Nội dung → Ảnh đính kèm → Mẫu ghim → Bảng đích → Người nhận → Đăng ẩn danh (đúng `docs/SPEC.v12.md` mục 2.5b)
- [ ] 5.9.4 Style lại input file, dropdown "Bảng đích"/"Người nhận", checkbox "Đăng ẩn danh" theo design token ở `docs/AGENT_RULES.v12.md` mục 8 — không dùng control mặc định trình duyệt
...
- [ ] 5.10.1 Migration: thêm cột `default_board_id` vào `members` (ALTER TABLE sau khi `boards` đã tồn tại — xem ghi chú (*) ở `docs/AGENT_RULES.v12.md` mục 4)
...
- [ ] 5.11.4 Cập nhật placeholder ô nội dung theo `docs/SPEC.v12.md` mục 2.5b
...
- [ ] 5.11.6 Cập nhật RPC `update_pin_content` theo đúng `docs/AGENT_RULES.v12.md` mục 5.2 — raise exception nếu sửa thành trống cả 2
...
## Phase 5.12 — Nút "Ẩn ghim" ngay trên board (Admin)

Phase này phụ thuộc Phase 6 (đã có cơ chế `is_hidden` + trang Kiểm duyệt) — nếu Phase 6 chưa xong, làm Phase 6 trước.

- [ ] 5.12.1 `<PinCard>` nhận thêm thông tin vai trò người xem hiện tại (đã có sẵn từ context/session — kiểm tra role trong `members`)
- [ ] 5.12.2 Nếu người xem là Admin và pin chưa `is_hidden`: thêm mục "Ẩn ghim" vào menu tuỳ chọn "..." đã có sẵn trên thẻ
- [ ] 5.12.3 Bấm "Ẩn ghim" → hỏi xác nhận 1 bước ngắn gọn → gọi lại đúng mutation/API đang dùng ở trang `/admin/pins` (không viết API mới)
- [ ] 5.12.4 Sau khi ẩn thành công: ghim biến mất khỏi board ngay (optimistic update hoặc refetch), không chuyển trang
- [ ] 5.12.5 Xác nhận: user thường (không phải Admin) không thấy mục "Ẩn ghim" ở bất kỳ ghim nào, kể cả ghim của chính mình
- [ ] 5.12.6 Commit — báo cáo: Admin ẩn được ghim trực tiếp từ board, user thường không thấy nút này, dữ liệu đồng bộ đúng với trang Kiểm duyệt

---

## Phase 5.13 — Board Admin (quyền admin giới hạn theo 1 board)

- [ ] 5.13.1 Migration: tạo bảng `board_admins` theo đúng `docs/AGENT_RULES.v12.md` mục 4
- [ ] 5.13.2 Cập nhật policy RLS cho `pins.is_hidden` và `boards` (skin/embed) theo đúng mục 5.5 — OR thêm điều kiện Board Admin, giữ nguyên phần company Admin đã có
- [ ] 5.13.3 UI gán Board Admin: trong form tạo board mới (Phase 3.3) và trong trang cài đặt board đã có (nếu chưa có trang cài đặt riêng cho từng board, tạo route `/[companySlug]/board/[boardId]/settings` chỉ chứa: đổi skin, bật/tắt embed, gán/gỡ Board Admin — không có nút giải thể phòng ban ở đây)
- [ ] 5.13.4 Route `/[companySlug]/board/[boardId]/settings` cho vào nếu company Admin HOẶC Board Admin của đúng board đó; Board Admin không thấy nút gán thêm Board Admin khác (chỉ company Admin gán được)
- [ ] 5.13.5 Cập nhật trang Kiểm duyệt (`/admin/pins`): cho Board Admin vào được, tự lọc chỉ hiện board mình được gán; company Admin vẫn thấy toàn bộ như cũ
- [ ] 5.13.6 Cập nhật nút "Ẩn ghim" trên board (Phase 5.12) và badge Kiểm duyệt (Phase 6): hiện cho cả Board Admin đúng board đó, không chỉ company Admin
- [ ] 5.13.7 Xác nhận Board Admin KHÔNG thấy nav "Cài đặt" (Thành viên/Phòng ban/Cài đặt công ty) nếu không đồng thời là company Admin, KHÔNG thao tác được ở board khác không được gán
- [ ] 5.13.8 Commit — báo cáo: gán Board Admin thành công, người được gán chỉ thao tác được đúng board mình quản lý, không đụng được board/thành viên/công ty khác

---

## Phase 5.14 — Icon "Cài đặt bảng" trên header (điểm truy cập còn thiếu)

- [ ] 5.14.1 Kiểm tra trạng thái hiện tại: route `/[companySlug]/board/[boardId]/settings` từ Phase 5.13 đã hoạt động chưa nếu gõ URL trực tiếp — nếu route chưa có, hoàn thành Phase 5.13 trước
- [ ] 5.14.2 Thêm icon "⚙️" trên header board (cạnh badge tên bảng + skin, VD "Bảng chung · Gỗ mộc ⚙️"), chỉ hiện với company Admin hoặc Board Admin của đúng board đang xem
- [ ] 5.14.3 Áp dụng cho **mọi board**, bao gồm cả Bảng chung công ty (`department_id = null`) — không chỉ board phòng ban
- [ ] 5.14.4 Bấm icon → điều hướng hoặc mở modal tới trang cài đặt board đã build ở Phase 5.13 (đổi skin, bật/tắt embed, gán Board Admin)
- [ ] 5.14.5 Commit — báo cáo kèm ảnh chụp: icon hiện đúng trên Bảng chung, đổi skin từ đây hoạt động

---

## Phase 5.15 — Trang Tài khoản cá nhân (mọi user)

- [ ] 5.15.1 Đổi nav "Đăng xuất" thành dropdown user menu (tên/avatar user) chứa 2 mục: "Tài khoản của tôi" và "Đăng xuất"
- [ ] 5.15.2 Trang `/[companySlug]/account`: form đổi tên hiển thị, gọi RPC `update_own_display_name` theo `docs/AGENT_RULES.v12.md` mục 5.6
- [ ] 5.15.3 Form đổi mật khẩu: nhập mật khẩu hiện tại + mật khẩu mới (2 lần) → xác nhận qua `signInWithPassword` rồi mới gọi `updateUser` theo đúng luồng mục 5.6 — báo lỗi rõ ràng nếu mật khẩu hiện tại sai
- [ ] 5.15.4 Hiển thị danh sách phòng ban user đang thuộc — chỉ đọc, không có nút sửa (đúng nguyên tắc chỉ Admin gán phòng ban)
- [ ] 5.15.5 Trang này không cần kiểm tra quyền Admin — bất kỳ user đã đăng nhập nào cũng vào được, chỉ thao tác trên dữ liệu của chính mình
- [ ] 5.15.6 Commit — báo cáo: đổi tên/mật khẩu thành công, sai mật khẩu cũ bị chặn đúng, danh sách phòng ban hiển thị đúng chỉ-đọc

---

## Phase 5.16 — Debug bug board không đổi màu khi đổi skin

- [ ] 5.16.1 Xác nhận `boards.skin` có lưu đúng giá trị mới trong DB sau khi đổi ở trang cài đặt board (kiểm tra trực tiếp trong Supabase Table Editor)
- [ ] 5.16.2 Kiểm tra component render nền board (`<Board>` theo `docs/AGENT_RULES.v1.md` mục 8) đang lấy `skin` từ đâu — props từ server component (đã refetch mới nhất) hay từ state/cache cũ chưa invalidate sau khi đổi cài đặt
- [ ] 5.16.3 Sửa để board luôn render đúng theo `boards.skin` mới nhất — có thể cần revalidate route/refetch sau khi lưu cài đặt board, hoặc kiểm tra key React có đang khiến component không re-render
- [ ] 5.16.4 Test cả 4 loại skin đổi qua lại, xác nhận nền board đổi đúng ngay sau khi lưu, không cần hard refresh trang
- [ ] 5.16.5 Commit — báo cáo nguyên nhân gốc đã tìm ra

---

## Phase 5.17 — Xác minh lại trang Tài khoản cá nhân (Phase 5.15)

- [ ] 5.17.1 Kiểm tra route `/[companySlug]/account` đã thực sự tồn tại và build thành công chưa (thử truy cập trực tiếp bằng URL)
- [ ] 5.17.2 Nếu route đã có nhưng không thấy điểm truy cập: xác nhận nav "Đăng xuất" đã được đổi thành dropdown user menu theo đúng task 5.15.1 chưa — nếu Cursor báo đã làm nhưng thực tế không thấy trên UI, kiểm tra lỗi render/conditional ẩn nhầm dropdown này
- [ ] 5.17.3 Nếu route chưa tồn tại: hoàn thành lại toàn bộ Phase 5.15 từ đầu
- [ ] 5.17.4 Commit — báo cáo rõ nguyên nhân: route thiếu, hay route có nhưng UI không dẫn tới được, kèm ảnh chụp xác nhận đã thấy và dùng được
...
- [ ] 6.4 Migration: thêm cột `reviewed_at` vào `pins` (xem `docs/AGENT_RULES.v12.md` mục 4). Khi Admin load trang `/admin/pins`, chạy update đánh dấu `reviewed_at = now()` cho các pin đang null theo đúng `docs/AGENT_RULES.v12.md` mục 5.3
...
## Phase 6b — Viết lại UX trang Kiểm duyệt (thay auto-mark bằng Duyệt/Từ chối)

- [ ] 6b.1 Đổi layout trang `/admin/pins` từ thẻ nhỏ xếp dọc sang **danh sách hàng ngang, full-width** — mỗi hàng hiện: tên bảng, người đăng (tên thật, kể cả ghim ẩn danh), nội dung, thời gian đăng
- [ ] 6b.2 Thêm 2 nút hành động trên mỗi hàng: **"Duyệt"** và **"Từ chối"** (thay cho link "Ẩn ghim" đơn lẻ hiện tại)
- [ ] 6b.3 "Duyệt" → gọi update chỉ `reviewed_at = now()` theo `docs/AGENT_RULES.v12.md` mục 5.3 — ghim giữ nguyên trên board, biến mất khỏi danh sách Kiểm duyệt
- [ ] 6b.4 "Từ chối" → gọi update `is_hidden = true, reviewed_at = now()` cùng lúc — ghim biến mất khỏi board công khai VÀ khỏi danh sách Kiểm duyệt
- [ ] 6b.5 **Xoá bỏ** logic cũ ở Phase 6.4 (tự động `UPDATE reviewed_at = now()` hàng loạt khi mở trang) — không còn tự động đánh dấu chỉ vì mở trang xem qua
- [ ] 6b.6 Badge đỏ ở nav "Kiểm duyệt" (Phase 6.5) chỉ giảm khi có ghim cụ thể được Duyệt hoặc Từ chối, không giảm khi chỉ mở trang
- [ ] 6b.7 Áp dụng đúng phạm vi quyền: company Admin thấy/xử lý mọi board, Board Admin chỉ thấy/xử lý board mình được gán (đúng mục 5.5)
- [ ] 6b.8 Commit — báo cáo kèm ảnh chụp layout mới, xác nhận Duyệt/Từ chối hoạt động đúng, badge cập nhật đúng theo hành động cụ thể
...
- [ ] 9.1 Rà lại toàn bộ UI copy (text hiển thị) đúng tiếng Việt, đúng giọng ấm áp như mô tả ở `docs/SPEC.v12.md` mục 3.1
...
- [ ] 9.6 Commit cuối — báo cáo tổng kết: những gì đã hoàn thành, những gì còn để Phase 2 (Odoo sync, upload nền tuỳ ý, export zip trả phí — xem `docs/SPEC.v12.md` mục 4)
