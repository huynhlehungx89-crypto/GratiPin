...
Đây là quy tắc bắt buộc khi build sản phẩm này. Đọc kỹ trước khi làm bất kỳ task nào trong `docs/TASKS.v4.md`. Nếu có mâu thuẫn giữa task cụ thể và rule ở đây, **rule ở đây luôn thắng**.

## 0. Quy ước versioning

Mỗi lần có thay đổi nghiệp vụ/thiết kế đáng kể, bộ 3 file `SPEC`, `AGENT_RULES`, `TASKS` được nâng version cùng lúc (VD: v2 → v3), file cũ giữ nguyên không sửa đè. Xem `docs/CHANGELOG.md` để biết version nào là mới nhất và thay đổi gì giữa các bản. **Luôn dùng version cao nhất khi làm việc**, trừ khi được yêu cầu khác.

---

## 1. Tổng quan sản phẩm

SaaS đa khách hàng (multi-tenant): nền tảng ghi nhận, biết ơn và lưu giữ kỷ niệm công ty, giao diện dạng bảng ghim (corkboard). Xem chi tiết tính năng & design tại `docs/SPEC.v4.md` (đính kèm cùng bộ này) — file đó là nguồn sự thật (source of truth) về nghiệp vụ.
...
Bất kỳ member nào trong công ty cũng được phép kéo-thả để đổi vị trí ghim (theo `SPEC.md` mục 2.6a), **nhưng không được để họ sửa được nội dung/ảnh/danh tính ghim của người khác qua cùng đường đó**. Vì vậy:
...
Tham chiếu đầy đủ tại `docs/SPEC.v4.md` mục 3. Tóm tắt bắt buộc:
...
- 4 board skin: wood / felt / linen / chalkboard — mỗi skin có bộ màu đinh ghim riêng (xem docs/SPEC.v4.md mục 3.4)
- 7 pin template: note / polaroid / floral / washi / garden / sunshine / love (xem docs/SPEC.v4.md mục 3.5)
...
**Không tự ý commit** nếu chưa hoàn thành 1 task trọn vẹn trong `docs/TASKS.v4.md` và chưa báo cáo lại kết quả (đã test được gì, còn thiếu gì) trước khi commit.

---

## 11. Thứ tự làm việc

Luôn làm theo đúng thứ tự trong `docs/TASKS.v4.md`, không nhảy cóc. Sau mỗi task, chạy thử (`npm run dev` hoặc test tương ứng) trước khi chuyển task tiếp theo. Nếu 1 task bị chặn vì thiếu quyết định nghiệp vụ chưa có trong `docs/SPEC.v4.md`, dừng lại và hỏi thay vì tự đoán.
