...
**Mới trong bản này:** không có thay đổi schema/RLS — chỉ làm rõ yêu cầu UI ở `docs/SPEC.v10.md` mục 2.6 (icon Cài đặt bảng phải hiển thị rõ, áp dụng cho mọi board kể cả Bảng chung).

Đây là quy tắc bắt buộc khi build sản phẩm này. Đọc kỹ trước khi làm bất kỳ task nào trong `docs/TASKS.v10.md`. Nếu có mâu thuẫn giữa task cụ thể và rule ở đây, **rule ở đây luôn thắng**.
...
SaaS đa khách hàng (multi-tenant): nền tảng ghi nhận, biết ơn và lưu giữ kỷ niệm công ty, giao diện dạng bảng ghim (corkboard). Xem chi tiết tính năng & design tại `docs/SPEC.v10.md` (đính kèm cùng bộ này) — file đó là nguồn sự thật (source of truth) về nghiệp vụ.
...
Bất kỳ member nào trong công ty cũng được phép kéo-thả để đổi vị trí ghim (theo `docs/SPEC.v10.md` mục 2.6a), **nhưng không được để họ sửa được nội dung/ảnh/danh tính ghim của người khác qua cùng đường đó**. Vì vậy:
...
Theo `docs/SPEC.v10.md` mục 2.5a: chỉ tác giả gốc (kể cả Admin cũng không được sửa ghim người khác) mới sửa được **content, image_url, template** — không sửa được `is_anonymous`, `recipient_member_id`, `board_id`.
...
- Đây chỉ là đánh dấu "đã lướt qua", không phải trạng thái duyệt/từ chối — ghim vẫn luôn public ngay khi đăng (đúng `docs/SPEC.v10.md` mục 2.9a), `reviewed_at` không ảnh hưởng gì tới việc ghim có hiển thị trên board hay không

### 5.4 Ẩn ghim ngay trên board (Admin) — tái dùng RLS đã có, không cần API mới

Theo `docs/SPEC.v10.md` mục 2.6d — đây thuần tuý là thêm 1 điểm truy cập UI trên board, gọi lại đúng logic set `pins.is_hidden = true` đã được bảo vệ bởi policy admin-only ở mục 5 (không phải tính năng backend mới):
...
Theo `docs/SPEC.v10.md` mục 2.3a. Nguyên tắc: mọi chỗ RLS hiện đang check `role = 'admin'` cho các hành động **giới hạn theo 1 board cụ thể** (ẩn ghim, đổi skin, bật/tắt embed) cần OR thêm điều kiện Board Admin đúng board đó. Các hành động **không giới hạn theo board** (quản lý members, departments, company settings) **giữ nguyên chỉ company Admin**, không mở rộng cho Board Admin.
...
Tham chiếu đầy đủ tại `docs/SPEC.v10.md` mục 3. Tóm tắt bắt buộc:
...
- 4 board skin: wood / felt / linen / chalkboard — mỗi skin có bộ màu đinh ghim riêng (xem docs/SPEC.v10.md mục 3.4)
- 7 pin template: note / polaroid / floral / washi / garden / sunshine / love (xem docs/SPEC.v10.md mục 3.5)
...
**Không tự ý commit** nếu chưa hoàn thành 1 task trọn vẹn trong `docs/TASKS.v10.md` và chưa báo cáo lại kết quả (đã test được gì, còn thiếu gì) trước khi commit.

---

## 11. Thứ tự làm việc

Luôn làm theo đúng thứ tự trong `docs/TASKS.v10.md`, không nhảy cóc. Sau mỗi task, chạy thử (`npm run dev` hoặc test tương ứng) trước khi chuyển task tiếp theo. Nếu 1 task bị chặn vì thiếu quyết định nghiệp vụ chưa có trong `docs/SPEC.v10.md`, dừng lại và hỏi thay vì tự đoán.
